/* Generated By:JJTree: Do not edit this line. AST_Declaration_EnumBody_Const.java Version 6.0 */
/* JavaCCOptions:MULTI=true,NODE_USES_PARSER=false,VISITOR=false,TRACK_TOKENS=true,NODE_PREFIX=AST,NODE_EXTENDS=NodeSetter,NODE_FACTORY=,SUPPORT_CLASS_VISIBILITY_PUBLIC=true */
package com.slang.frontend.parser;

public
class AST_Declaration_EnumBody_Const extends SimpleNode {
  public AST_Declaration_EnumBody_Const(int id) {
    super(id);
  }

  public AST_Declaration_EnumBody_Const(ScriptlangParser p, int id) {
    super(p, id);
  }

}
/* JavaCC - OriginalChecksum=46d31a594d5717e58ada83ce046cda4a (do not edit this line) */
